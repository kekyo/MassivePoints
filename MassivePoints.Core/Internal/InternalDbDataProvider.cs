...
internal static class InternalDbDataProvider
{
    ...
    /// </summary>
    /// <param name="provider">`DbDataProvider`</param>
    ...
    public static async ValueTask CreateSQLiteTablesAsync<TValue>(
        DbDataProvider<TValue> provider,
        bool dropToCreate, CancellationToken ct = default)
    {
        ...
        var childBoundCount = InternalBound.GetChildBoundCount(provider.Configuration.Entire);
        var dimensionAxisCount = InternalBound.GetDimensionAxisCount(provider.Configuration.Entire);
